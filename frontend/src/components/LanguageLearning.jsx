...
const SKILL_DESCRIPTIONS = {
  beginner:     "just starting out — no worries!",
  intermediate: "getting comfortable with the basics",
  advanced:     "ready for some real challenges",
};
...
export default function LanguageLearning({ onPractice }) {
  ...
  // Flat ordered list of all topics so we can navigate prev/next
  ...
  // ── Fetch the curriculum for the chosen language ──
  const handleSelectLanguage = async (lang) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Couldn't build the curriculum — please try again.");
    } finally {
  ...
  const handleOpenTopic = async (topic, moduleTitle) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Couldn't load the lesson — please try again.");
    } finally {
  ...
  // ── Mark a lesson as done and optionally move to the next ──
  ...
  if (screen === "pick") {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Pick a language to learn</h2>
          <p className="text-sm text-gray-500 mt-1">
            Choose a language, tell us your level, and we'll build you a full curriculum with lessons, examples and practice challenges — all generated by AI, just for you.
          </p>
        </div>

        {/* Skill level selector */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">I'm a…</p>
          <div className="flex gap-2">
            {SKILL_LEVELS.map((level) => (
              ...
              >
                {level}
                {skillLevel === level && (
                  <span className="ml-1.5 opacity-80 text-xs font-normal hidden md:inline">
          ...
          {LANGUAGES.map((lang) => {
            ...
            return (
              <button
                ...
                className="bg-white border border-gray-200 hover:border-blue-400 hover:shadow-md rounded-xl p-4 text-left transition-all group hover:-translate-y-0.5"
              >
                ...
                {done > 0 ? (
                  <p className="text-xs text-emerald-600 font-medium">✓ {done} topic{done > 1 ? "s" : ""} done</p>
                ) : (
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <button onClick={() => setScreen("pick")} className="text-sm text-gray-400 hover:text-gray-600 flex items-center gap-1">
            ← All languages
          </button>
        ...
        {loadingCurr && (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-sm">Building your personal curriculum…</p>
            <p className="text-xs mt-1 text-gray-300">This takes a few seconds ☕</p>
          </div>
        )}

        {error && <div className="p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm">⚠️ {error}</div>}
          ...
          <>
            <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3 shadow-sm">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                <span>Topics: {curriculum.totalTopics}</span>
                <span>Estimated time: ~{curriculum.estimatedHours} hours</span>
                <span className="capitalize">Level: {skillLevel}</span>
              </div>
              {progress.total > 0 && (
                <div>
                  <div className="flex justify-between text-xs text-gray-500 mb-1">
                    <span>{progress.done} of {progress.total} topics completed</span>
                ...
                <div key={mod.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-3">
                      <span className="text-xs text-gray-400">{mod.topics?.length} lessons</span>
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                                done ? "bg-emerald-500 border-emerald-500" : "border-gray-300 group-hover:border-blue-400"
                              }`}>
                                {done && (
          ...
          <>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-blue-600 mt-1">Objective: {lesson.objective}</p>
                </div>
                {done && (
                  <span className="shrink-0 text-xs bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-1 rounded-full font-medium">
                    ✓ Completed
                  </span>
                )}
              </div>
            ...
            {lesson.commonMistakes?.length > 0 && (
              <div className="bg-orange-50 border border-orange-200 rounded-xl p-5 shadow-sm">
                <h3 className="text-sm font-semibold text-orange-800 mb-3">Common mistakes to watch out for</h3>
            ...
            {lesson.summary && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 shadow-sm">
                <h3 className="text-sm font-semibold text-blue-800 mb-2">Quick recap</h3>
            ...
            {lesson.practicePrompt && (
              <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3 shadow-sm">
                <h3 className="text-sm font-semibold text-gray-800">Try it yourself</h3>
                ...
                <button
                  ...
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-lg text-sm font-medium transition-all hover:shadow-sm"
                >
                  Practise This Topic →
                </button>
              </div>
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
              ...
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between gap-3">
                {!done ? (
                  <>
                    <p className="text-sm text-gray-600">Finished reading? Mark this lesson done!</p>
                    <button
                      onClick={() => markComplete(false)}
                      className="shrink-0 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                    >
                      Mark Complete ✓
                    </button>
                  </>
                ) : (
                  <p className="text-sm text-emerald-700 font-medium">✓ You've completed this lesson — great work!</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 divide-x divide-gray-100">
                ...
                >
                  <span className="text-xs text-gray-400 mb-0.5">← Previous lesson</span>
                ...
                >
                  <span className="text-xs text-gray-400 mb-0.5">Next lesson →</span>
